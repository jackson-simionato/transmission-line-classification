...
class FeatureExtractionService:
    ...
    def __init__(
        ...
        logger.info(f"Feature extraction service initialized")
        logger.info(f"CNN embeddings: {'enabled' if include_cnn else 'disabled'}")
        if include_cnn:
            logger.info(f"CNN model: {cnn_model} on {self.device}")
    
    def _get_default_features(self, segment_id: int, class_label: Optional[str]) -> Dict[str, Union[float, int, str]]:
        """Return default zero features for invalid polygons"""
        return {
            'segment_id': segment_id,
            'class_label': class_label or 'unlabeled',
            'rgb_mean_r': 0.0, 'rgb_mean_g': 0.0, 'rgb_mean_b': 0.0,
            'rgb_std_r': 0.0, 'rgb_std_g': 0.0, 'rgb_std_b': 0.0,
            'hsv_mean_h': 0.0, 'hsv_mean_s': 0.0, 'hsv_mean_v': 0.0,
            'hsv_std_h': 0.0, 'hsv_std_s': 0.0, 'hsv_std_v': 0.0,
            'compactness': 0.0, 'solidity': 0.0, 'aspect_ratio': 0.0,
            'elongation': 0.0, 'rectangularity': 0.0, 'convexity': 0.0, 'circularity': 0.0,
            'glcm_contrast': 0.0, 'glcm_dissimilarity': 0.0, 'glcm_homogeneity': 0.0,
            'glcm_energy': 0.0, 'glcm_correlation': 0.0, 'glcm_asm': 0.0
        }
    ...
    def _initialize_cnn_model(self):
        ...
        try:
            if self.cnn_model_name == "mobilenet_v3_small":
                self.cnn_model = timm.create_model('mobilenetv3_small_100', pretrained=True)
                self.cnn_model.eval()
    ...
    def extract_spectral_features(self, image_patch: np.ndarray, mask: np.ndarray) -> Dict[str, float]:
        """
    ...
    def extract_texture_features(self, image_patch: np.ndarray, mask: np.ndarray) -> Dict[str, float]:
        """
    ...
    def extract_features_from_polygon(
        ...
        if len(rows) == 0:
            logger.warning(f"No polygon pixels found in full mask for segment {segment_id}")
        ...
        image_patch = tile_image[actual_min_row:actual_max_row, actual_min_col:actual_max_col]
    ...
    def process_tile(
        ...
        logger.info(f"Extracted features for {len(features_list)} polygons in {tile_name}")
