...
class SegmentPostprocessingService:
    ...
    def __init__(self, image_height: int, image_width: int):
        ...
        logger.info(
            f"Post-processing service initialized for {image_width}x{image_height} images"
        )

    def fill_gaps_with_background_class(
        self, segments: List[Dict], min_gap_size: int = 500
    ) -> List[Dict]:
        """
        ...
        new_segments = []
        segment_id_offset = (
            max([s["segment_id"] for s in segments]) + 1 if segments else 0
        )
    ...
    def fill_gaps_with_superpixels(
        self, segments: List[Dict], image: np.ndarray, n_segments: int = 200
    ) -> List[Dict]:
        """
        ...
        superpixels = slic(
            gap_image, n_segments=n_segments, compactness=10.0, mask=gaps_mask
        )
    ...
    def smooth_segment_boundaries(
        self, segments: List[Dict], kernel_size: int = 5, iterations: int = 1
    ) -> List[Dict]:
        """
    ...
    def remove_large_overlapping_segments(
        ...
        logger.info(
            f"  Criteria 2: Segments overlapping {min_overlapped_segments}+ others significantly"
        )
        ...
        for i, seg_i in enumerate(segments):
            ...
            for j, seg_j in enumerate(segments):
                ...
                overlap = np.logical_and(
                    seg_i["segmentation"], seg_j["segmentation"]
                ).sum()

                if overlap > 0:
                    # Calculate overlap ratio relative to the smaller segment
                    overlap_ratio_to_smaller = overlap / min(
                        seg_i["area"], seg_j["area"]
                    )
            ...
            if seg_i["area"] > total_pixels * max_segment_area_ratio:
                if len(overlaps) > 0:
                    logger.info(
                        f"  Segment {seg_i['segment_id']}: VERY LARGE ({seg_i['area']} pixels = "
            ...
            if len(significant_overlaps) >= min_overlapped_segments:
                ...
                total_overlap_pixels = sum(
                    o["overlap_pixels"] for o in significant_overlaps
                )
                ...
                for o in significant_overlaps:
                    j = o["index"]
                    overlap_with_j = np.logical_and(
                        seg_i["segmentation"], segments[j]["segmentation"]
                    )
                ...
                if bbox_width > 0 and bbox_height > 0:
                    aspect_ratio = max(bbox_width, bbox_height) / min(
                ...
                logger.info(
                    f"  Segment {seg_i['segment_id']} ({seg_i['area']} pixels) overlaps {len(significant_overlaps)} segments "
        ...
        cleaned_segments = [
            seg for i, seg in enumerate(segments) if i not in segments_to_remove
        ]
    ...
    def remove_overlaps(self, segments: List[Dict]) -> List[Dict]:
        ...
        original_segments = [
            s
            for s in segments
            if not s.get("is_gap_fill", False)
            and not s.get("is_superpixel_fill", False)
        ]
        gap_segments = [
            s
            for s in segments
            if s.get("is_gap_fill", False) or s.get("is_superpixel_fill", False)
        ]
        ...
        logger.info(
            f"Removing overlaps: {len(original_segments)} original, {len(gap_segments)} gap-fill segments"
        )
        ...
        logger.info(
            f"After overlap removal: kept {len(cleaned_gap_segments)}/{len(gap_segments)} gap-fill segments"
        )
    ...
    def calculate_coverage_stats(self, segments: List[Dict]) -> Dict:
        ...
        superpixel_count = sum(
            1 for s in segments if s.get("is_superpixel_fill", False)
        )
        ...
        stats = {
            ...
            "mean_segment_area": float(np.mean([s["area"] for s in segments]))
            if segments
            else 0,
            "median_segment_area": float(np.median([s["area"] for s in segments]))
            if segments
            else 0,
        }
    ...
    def process_complete(
        ...
        if fill_gaps:
            if use_superpixels and image is not None:
                processed = self.fill_gaps_with_superpixels(
                    processed, image, n_superpixels
                )
            else:
                if use_superpixels and image is None:
                    logger.warning(
                        "Superpixels requested but no image provided, using simple fill"
                    )
                processed = self.fill_gaps_with_background_class(
                    processed, min_gap_size=min_gap_size
                )
        ...
        if smooth_boundaries:
            processed = self.smooth_segment_boundaries(
                processed, kernel_size=smoothing_kernel_size
            )
