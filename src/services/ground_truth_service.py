...
class GroundTruthService:
    ...
    def __init__(self, min_distance_meters: float = 10.0, random_seed: int = 42):
        ...
        logger.info(f"GroundTruthService initialized: min_distance={min_distance_meters}m, seed={random_seed}")
    ...
    def calculate_point_weights(segments: gpd.GeoDataFrame) -> np.ndarray:
        ...
        logger.debug(f"Calculated weights for {len(segments)} segments (min: {weights.min():.4f}, max: {weights.max():.4f})")
        return weights
    
    @staticmethod
    def generate_random_point_in_polygon(polygon, max_attempts: int = 100) -> Optional[Point]:
        """
        ...
        logger.warning(f"Failed to generate point in polygon after {max_attempts} attempts")
        return None
    
    @staticmethod
    def is_valid_distance(point: Point, existing_points: List[Point], min_distance: float) -> bool:
        """
    ...
    @staticmethod
    def create_point_attributes(point: Point, segment: Dict, sample_id: str, tile_name: str) -> Dict:
        """
    ...
    def generate_tile_samples(self, segments_gdf: gpd.GeoDataFrame, num_points: int = 15) -> gpd.GeoDataFrame:
        """
        ...
        logger.info(f"Generating {num_points} points from {len(segments_list)} segments")
        ...
        while len(points) < num_points and attempts < max_attempts:
            ...
            if self.is_valid_distance(point, point_geometries, self.min_distance):
                ...
                attributes = self.create_point_attributes(point, segment, sample_id, tile_name)
        ...
        if len(points) < num_points:
            logger.warning(f"Only generated {len(points)}/{num_points} points (attempts: {attempts})")
        
        # Create GeoDataFrame
        if points:
            points_gdf = gpd.GeoDataFrame(points, geometry=point_geometries, crs=segments_gdf.crs)
    ...
    def generate_all_tiles_samples(self, input_dir: Path, output_dir: Path, 
                                 points_per_tile: int = 15) -> Dict:
        """
        ...
        tile_dirs = [d for d in input_dir.iterdir() 
        ...
        for tile_dir in tile_dirs:
            ...
            if not points_gdf.empty:
                ...
                tile_output_path = output_dir / "by_tile" / f"{tile_name}_ground_truth_points.geojson"
        ...
        logger.info(f"Sampling complete: {summary['successful_tiles']}/{summary['total_tiles']} tiles, {total_points} total points")
