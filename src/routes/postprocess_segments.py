...
class SegmentPostprocessingPipeline:
    ...
    def __init__(
        ...
        self.original_images_dir = Path(original_images_dir) if original_images_dir else None
    ...
    def load_segments_from_geojson(
        ...
        if 'transform' in metadata:
            ...
            transform = rasterio.Affine(pixel_width, 0.0, bounds[0], 
                                       0.0, -pixel_height, bounds[3])
        else:
            logger.warning("No transform or bounds in metadata, using identity transform")
    ...
    def save_processed_geojson(
            ...
            for geom, value in rasterio_shapes(mask, transform=transform):
                if value == 1:
                    polygons.append({
                        ...
                        'is_superpixel_fill': segment.get('is_superpixel_fill', False),
                        'is_border_fill': segment.get('is_border_fill', False),
                        'suggested_class': segment.get('suggested_class', 'unknown')
                    })
        ...
        if close_boundaries and len(gdf) > 0:
            ...
            if not gaps.is_empty:
                ...
                if gap_polygons:
                    gap_data = []
                    segment_id_offset = max(gdf['segment_id']) + 1 if len(gdf) > 0 else 0
                    
                    for i, gap_geom in enumerate(gap_polygons):
                        gap_data.append({
                            'geometry': gap_geom,
                            'segment_id': segment_id_offset + i,
                            'area_pixels': int(gap_geom.area / (transform.a * abs(transform.e))),  # Approximate pixel area
                            'predicted_iou': 0.5,
                            'stability_score': 0.5,
                            'is_gap_fill': False,
                    ...
                    logger.info(f"Added {len(gap_polygons)} gap polygons to close tile boundaries")
    ...
    def postprocess_tile(self, tile_dir: Path) -> Dict:
        ...
        self.save_processed_geojson(processed_segments, metadata, geojson_output, transform, config['close_boundaries'])
        ...
        logger.info(f"✓ Completed {tile_name}: {initial_stats['coverage_percent']:.1f}% → {final_stats['coverage_percent']:.1f}% coverage")
    ...
    def process_directory(self, input_dir: Path) -> Dict:
        ...
        tile_dirs = [d for d in input_dir.iterdir() if d.is_dir() and (d / 'segments.geojson').exists()]
        ...
        if results:
            summary = {
                ...
                'average_coverage_before': np.mean([r['coverage_before'] for r in results]),
                'average_coverage_after': np.mean([r['coverage_after'] for r in results]),
                'tiles': results
            }
        else:
            summary = {
        ...
        logger.info("="*60)
        logger.info(f"Processed: {summary['processed_tiles']}/{summary['total_tiles']} tiles")
        if results:
            logger.info(f"Avg coverage before: {summary['average_coverage_before']:.1f}%")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input-dir",
        required=True,
        help="Directory with SAM segment outputs"
    )
    parser.add_argument(
        "--output-dir",
        default=None,
    ...
    parser.add_argument(
        "--smooth-boundaries",
        action="store_true",
        help="Smooth segment boundaries"
    )
    parser.add_argument(
        "--no-remove-overlaps",
    ...
    if args.output_dir is None:
        args.output_dir = str(Path(args.input_dir).parent / (Path(args.input_dir).name + '_processed'))
